...
# NOTE: Hardcoding a path like "C:/Users/Lenovo/Downloads/..." only works on YOUR
# machine. If you deploy this to Streamlit Community Cloud or share it with your
# teacher, that path won't exist on their computer and the app will crash.
# This version looks for the CSV next to the script first, and falls back to a
# file uploader if it isn't found — so it works everywhere.

DEFAULT_FILENAME = "Atlantic_United_States (1).csv"
...
# Try to find the CSV automatically (same folder as this script), else ask for upload
...
if os.path.exists(default_path):
    ...
    st.sidebar.warning(f"⚠️ '{DEFAULT_FILENAME}' not found next to the script.")
    uploaded = st.sidebar.file_uploader("Upload Atlantic CSV", type=["csv"])
    if uploaded is None:
        st.title("🎵 Atlantic RC – US Top 50 Analytics")
        st.info("Please upload the CSV file from the sidebar to continue.")
        st.stop()
...
if page == "🏠 Overview":
    ...
    with col3:
        ...
        ax.pie(ec.values,
               labels=['Non-Explicit', 'Explicit'] if not ec.index[0] else ['Explicit', 'Non-Explicit'],
    ...
    with col4:
        ...
        ax.bar(alb.index, alb.values, color=['#1DB954', '#3498DB', '#9B59B6'][:len(alb)])
